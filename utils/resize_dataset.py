...
def change_dirs(paths):
    new_paths = []
    for path in paths:
        new_path = path.replace("/home/adam_chlopowiec/data/eye_image_classification/data_resized", "../input/datasetresized/data_resized/data_resized")
...
data = pd.read_csv('resized_collected_data_splits.csv')
paths = data['Path']
...
data['Path'] = new_paths
data.to_csv('kaggle_data_splits.csv')